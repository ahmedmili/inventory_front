...
export default function EmployeeDashboard() {
  ...
  const isStockKeeper = roleCode === 'STOCK_KEEPER';
  
  // Check permissions for actions
  const canCreateProduct = hasPermission(user, 'products.create');
  ...
  const canReadReservations = hasPermission(user, 'reservations.read');
    ...
    <div className="px-4 py-6 sm:px-0">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          Bienvenue, {user.firstName} {user.lastName} • Vue d'ensemble de votre espace de travail
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2 mb-8">
        <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-100">
          ...
          <div className="grid grid-cols-2 gap-4">
            {/* Sales - All employees can create */}
            <Link
              href="/sales/new"
            ...
            {/* Customers - All employees can create */}
            <Link
              href="/customers/new"
            ...
            {/* Reservations - All employees can create */}
            {canCreateReservation && (
              ...
              >
                ...
                <span className="text-sm font-medium text-gray-700">Réserver Produit</span>
              </Link>
            )}
            
            {/* Products - Only if has permission */}
            {canCreateProduct && (
              <Link
                href="/products/new"
            ...
            {/* Purchases - Only if has permission */}
            {canCreatePurchase && (
              <Link
                href="/purchases/new"
                className="flex flex-col items-center justify-center p-4 bg-orange-50 rounded-lg hover:bg-orange-100 transition-colors group"
              >
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-100">
          ...
          <div className="space-y-3">
            ...
            {/* Reservations - Show if user can read reservations */}
            {canReadReservations && (
              <Link
                href="/reservations"
                className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors group"
              >
                <div className="flex items-center">
                  ...
                  <span className="text-sm font-medium text-gray-700">Mes Réservations</span>
                </div>
                <svg className="w-4 h-4 text-gray-400 group-hover:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                </svg>
            ...
            {/* Warehouses - Show if user can read warehouses */}
            {hasPermission(user, 'warehouses.read') && (
              ...
              >
                <div className="flex items-center">
                  ...
                  <span className="text-sm font-medium text-gray-700">Entrepôts</span>
                </div>
                <svg className="w-4 h-4 text-gray-400 group-hover:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                </svg>
          ...
          <div className="flex items-center">
            ...
            <div className="flex-1">
              ...
              <p className="text-amber-700 mt-1">
                {stats.lowStock} produit{stats.lowStock > 1 ? 's' : ''} avec un stock faible nécessitant une attention.
              </p>
            </div>
            <Link
              href="/products"
        ...
        <div className="space-y-4">
          <div className="flex items-start">
            ...
            <div className="ml-4 flex-1">
              <h3 className="text-sm font-medium text-gray-900">Vos Permissions</h3>
              <p className="text-sm text-gray-600 mt-1">
                {isStockKeeper 
                  ? "En tant que Stock Keeper, vous pouvez gérer les produits, le stock et les entrepôts."
                  : "En tant qu'Employé, vous pouvez consulter les produits, créer des ventes, gérer les clients et réserver des produits. Vous ne pouvez pas créer, modifier ou supprimer des produits."}
              </p>
            </div>
          </div>
          <div className="flex items-start">
            ...
            <div className="ml-4 flex-1">
              <h3 className="text-sm font-medium text-gray-900">Actions Disponibles</h3>
              <p className="text-sm text-gray-600 mt-1">
                {isStockKeeper
                  ? "Vous pouvez créer des produits, gérer les achats, et effectuer des mouvements de stock."
                  : "Vous pouvez créer des ventes, ajouter de nouveaux clients, réserver des produits pour des projets, et consulter les informations de stock. Les réservations permettent de mettre de côté des produits sans les retirer définitivement du stock."}
              </p>
            </div>
          </div>
        </div>
